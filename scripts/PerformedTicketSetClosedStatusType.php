...
/** Наименование записи */
const RECORD = 'Состояние: Закрыт';

/** Наименование записи */
const NOTE = 'Закрыт автоматически';
...
function getTickets(): ?array
{
    $sth = getConnect()->prepare("
        ...
        FROM
            `tickets_tickets`
        WHERE
            `tickets_tickets`.`statustypeid` = 4
                AND
            `tickets_tickets`.`performingdate` > DATE_SUB(
                NOW(),
...
function updateTicket(string $ticketId): bool
{
    ...
    $sth = $sth ?? getConnect()->prepare("
        UPDATE
            `tickets_tickets`
        SET
            `tickets_tickets`.`closingdate` = NOW(),
            `tickets_tickets`.`closed_by_stuffid` = :staffId,
