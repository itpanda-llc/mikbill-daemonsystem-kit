 ...
 * @link https://github.com/itpanda-llc/mikbill-deviceview-php-api
 */
const API_REASON_PARAM  = ['reason' => 'nas'];

/**
 * Параметр "Тип устройств"
 * @link https://github.com/itpanda-llc/mikbill-deviceview-php-api
 */
const API_TYPE_PARAM  = ['type' => 'mikrotik'];
...
/** Имя таблицы БД */
const TABLE_NAME = 'nas';

/** Параметр "Секрет" */
const NAS_SECRET = 'secret';
...
/**
 * @param string $nasName Сервер
 */
function addServer(string $nasName): void
{
    static $sth;
    ...
    $sth->bindParam(':nasName', $nasName);
    $sth->bindValue(':secret', NAS_SECRET);
...
try {
    foreach ($j->result as $v) addServer((string) $v[0]);
